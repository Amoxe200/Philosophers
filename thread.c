...
/*   Updated: 2021/11/02 18:04:37 by aaqari           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned long long to_ms(struct timeval timer)
{
    ...
    x = (((unsigned long long)timer.tv_sec) * 1000) + (((unsigned long long)timer.tv_usec) / 1000);
    return (x);
}
...
void    print(char *str, t_philo *philo)
{
    ...
    printf("%d %d %s\n" ,(int)(to_ms(now) - to_ms(philo->info->base)), philo->philo_id + 1, str);
    if (str[3] !='D')
